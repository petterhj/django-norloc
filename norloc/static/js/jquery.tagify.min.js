function M2MField(input, template, settings) {
    // Settings
    // this.settings = this.extend({}, this.default_settings, settings);    
    this.settings = Object.assign({}, this.default_settings, settings);

    // Template
    this.object_template = template;

    // Input
    this.element = this.build(input);
}

M2MField.prototype = {
    // Default settings
    default_settings: {
        object_class: 'm2m_object'
    },
    ...
    build: function(input) {
        var element = this.parseHTML('<div class="m2m_input"></div>');
    ...
    // Add object
    addObject: function(object_data) {
        var object = this.parseHTML('<div>');
            object.classList.add(this.settings.object_class);
        
        var close = this.parseHTML('<span class="close"></span>')
        // object.prepend(close);
        object.appendChild(close);

        var content = this.parseHTML(this.object_template(object_data));
...
;(function(root, factory) {
    ...
    Tagify.prototype = {
                    ...
                    switch (e.key) {
                        case 'Backspace':
                            if (s == "" || s.charCodeAt(0) == 8203) {
                                ...
                                if (this.settings.backspace === true) this.removeTag(lastTag);
                                else if (this.settings.backspace == 'edit') this.editTag(lastTag);
                            }
        ...
        input: {
            ...
            autocomplete: {
                suggest: function suggest(s) {
                    if (!s || !this.input.value) this.DOM.input.removeAttribute("data-suggest");
                    else this.DOM.input.setAttribute("data-suggest", s.substring(this.input.value.length));
                },
                set: function set(s) {
        ...
        validateTag: function validateTag(s) {
            ...
            if (!value) result = this.TEXTS.empty;
            else if (maxTagsExceed) result = this.TEXTS.exceed; // check if pattern should be used and if so, use it to test the value
            ...
            else if (!this.settings.duplicates && this.isTagDuplicate(value) !== -1) result = this.TEXTS.duplicate;
            else if (this.isTagBlacklisted(value) || this.settings.enforceWhitelist && !this.isTagWhitelisted(value)) result = this.TEXTS.notAllowed;
            return result;
        },
        ...
        addTags: function addTags(tagsItems, clearInput) {
            ...
            function appendTag(tagElm) {
                ...
                if (insertBeforeNode === this.DOM.input) this.DOM.scope.insertBefore(tagElm, insertBeforeNode);
                else this.DOM.scope.appendChild(tagElm);
            }
